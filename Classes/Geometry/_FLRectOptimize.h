...
BOOL FLRectWidthIsOptimizedForView(FLRect r) {
    return r.size.width == 0 || (FLRectIsIntegral(r) && (fmod(r.origin.x + r.size.width, 2.0f) == 0));
}

FL_SHIP_ONLY_INLINE
BOOL FLRectHeightIsOptimizedForView(FLRect r) {
    return r.size.height == 0 || (FLRectIsIntegral(r) && (fmod(r.origin.y + r.size.height, 2.0f) == 0));
}
...
FLSize FLSizeOptimizeForView(FLSize aSize) {
	...
	if(fmod(size.width, 2.0f) != 0) size.width += 1.0f;
	if(fmod(size.height, 2.0f) != 0) size.height += 1.0f;
	return size;
}
