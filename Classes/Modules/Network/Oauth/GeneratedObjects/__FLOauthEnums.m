...
- (NSInteger) lookupString:(NSString*) inString {
    ...
    if(!num) { FLThrowErrorCode_v(FLFrameworkErrorDomainName, FLErrorUnknownEnumValue, [NSString stringWithFormat:(NSLocalizedString(@"Unknown enum value (case sensitive): %@", nil)), inString]); } 
    return [num intValue];
}
