...
//- (void) testReachability:(void (^)(FLReachability* monitor, BOOL isReachable)) resultBlock
//{
//    resultBlock(self, FLBitTest(self.currentReachabilityFlags, self.interestingFlags));
//}
//
//- (void) getCurrentFlags:(void (^)(FLReachability* monitor, SCNetworkReachabilityFlags flags)) resultBlock
//{
...
- (void) onReachabilityCallback:(SCNetworkReachabilityRef) target 
    ...
    SCNetworkReachabilityFlags newFlags = FLBitTest(flags, self.monitoredFlags);
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
	[((id) info) onReachabilityCallback:target flags:flags];
}

- (BOOL) testCurrentFlags:(SCNetworkReachabilityFlags) mask {
    return FLBitTest(self.currentReachabilityFlags, mask);
}
...
- (void) startMonitoring {
	...
	if(	!_isMonitoring &&
        ...
        _lastMonitoredFlags = FLBitAnd(self.currentReachabilityFlags, self.monitoredFlags);
		_isMonitoring = YES;
	}
}
...
- (BOOL) isReachable {
    return FLBitTest(self.currentReachabilityFlags, kSCNetworkReachabilityFlagsReachable);
}
