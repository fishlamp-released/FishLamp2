...
static void * const kViewControllerContentsDescriptorKey = (void*)&kViewControllerContentsDescriptorKey;
...
- (FLViewContentsDescriptor*) viewContentsDescriptor {
    FLViewContentsDescriptor* descriptor = objc_getAssociatedObject(self, kViewControllerContentsDescriptorKey);
    if(!descriptor) {
...
- (void) setViewContentsDescriptor:(FLViewContentsDescriptor*) viewContentsDescriptor {
     objc_setAssociatedObject(self, kViewControllerContentsDescriptorKey, viewContentsDescriptor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
   ...
   switch(contents.bottom)
	{
		case FLViewContentItemNavigationBarAndStatusBar:
		case FLViewContentItemToolbarAndStatusBar:
			FLAssertFailed(@"can't have status bar on bottom");
			break;
