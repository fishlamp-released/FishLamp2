	...
	finished:(NSNumber *)finished context:(void *)context
{
	...
	[UIView beginAnimations:@"slide" context:FLReturnRetained(viewController)];
...
- (FLButtonbarView*) buttonbar
{   
    id view = self.topBarView;
	return [view isKindOfClass:[FLButtonbarView class]] ? (FLButtonbarView*) view : nil;
}
...
- (void) _prepareToShowButtonBar:(FLButtonbarView*) buttonbar
{
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
	...
	{
		if([subview isKindOfClass:[FLButtonbarView class]] && !subview.hidden)
		{
			if(animated)
			{
    ...
    {
        FLButtonbarView* buttonbar = viewController.buttonbar;
        if(buttonbar)
        {	
    ...
    viewController.dismissHandler = ^(UIViewController* dismissMe, BOOL dismissAnimated) {
        ...
        {
            FLAssert(navController.visibleViewController == dismissMe, @"popping wrong view controller");
...
- (void) _viewControllerWasPopped:(UIViewController*) viewController animated:(BOOL) animated
{
	...
	FLButtonbarView* topButtonbar = self.topViewController.buttonbar;
	if(topButtonbar)
	{	
