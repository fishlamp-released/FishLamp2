...
//    FLAssert([[FLWindow topWindow] isKindOfClass:[FLWindow class]], @"not a gtwindow");
...
- (void) addDragResponder:(id) dragResponder
{
    ...
    FLAssert([_draggableObjects indexOfObject:dragResponder] == NSNotFound, @"responder already installed");
...
- (void) addDragDestination:(id) destination 
{
    ...
    FLAssert([_dragDestinations indexOfObject:destination] == NSNotFound, @"destination already installed");
...
{
    FLAssert(self.isDragging, @"not dragging");
...
{
    FLAssertIsNotNil(self.hostView);
    FLAssert(!self.isDragging, @"dragging");
    self.dragging = YES;
...
+ (FLDragControllerDragDestination*) dragDestination:(FLCallback) callback
{
