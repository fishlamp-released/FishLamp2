...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	NSCalendar *calendar = FLReturnAutoreleased([[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar]);
	calendar.firstWeekday = 2; //Monday
	NSDate *now = [NSDate date];

	NSDateComponents *components = FLReturnAutoreleased([[NSDateComponents alloc] init]);
...
- (NSString *) replaceColonsInString:(NSString *)timeFormat withTimeSeparator:(unichar)timeSep {
	if (timeSep != ':') {
		NSMutableString *timeFormatMutable = FLReturnAutoreleased([timeFormat mutableCopy]);
