...
+ (NSString*) URLString:(NSString*) url params:(NSString*) firstParameter, ... 
{
	if(firstParameter)
	{
		NSMutableString* outUrl = GtReturnAutoreleased([url mutableCopy]);
		BOOL isFirst = YES;
...
{
	NSMutableString* outUrl = GtReturnAutoreleased([self mutableCopy]);
	BOOL isFirst = YES;
	NSString* key = firstParameter;
	id obj = nil;
...
{
	return GtReturnAutoreleased((NSString *) CFURLCreateStringByAddingPercentEscapes(
			kCFAllocatorDefault, 
			(CFStringRef)self,
			NULL, // escape all
			(CFStringRef)@";/?:@&=$+{}<>,'\"",
...
{
	return GtReturnAutoreleased((NSString *) CFURLCreateStringByReplacingPercentEscapesUsingEncoding(
		kCFAllocatorDefault, 
		(CFStringRef)self,
		CFSTR(""), 
		CFStringConvertNSStringEncodingToEncoding(encoding)) );
