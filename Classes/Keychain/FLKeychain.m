								...
								error: (NSError **) error 
{
	...
	@synchronized(self) {
		...
		NSArray *keys = [[NSArray alloc] initWithObjects: (__fl_bridge id) kSecClass, (__fl_bridge id) kSecAttrAccount, (__fl_bridge id) kSecAttrService, nil];
        
        
		NSArray *objects = [[NSArray alloc] initWithObjects: (__fl_bridge id) kSecClassGenericPassword, username, serviceName, nil];
		...
		[attributeQuery setObject: (id) kCFBooleanTrue forKey:(__fl_bridge id) kSecReturnAttributes];

		NSDictionary *attributeResult = NULL;

		OSStatus status = SecItemCopyMatching((__fl_bridge CFDictionaryRef) attributeQuery, (__fl_bridge CFTypeRef *) &attributeResult);
