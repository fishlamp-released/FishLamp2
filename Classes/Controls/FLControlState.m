...
- (void) setControlState:(FLControlStateMask) state {
    if( state != _controlState && FLBitTest(_enabledControlStates, state)) {
...
- (BOOL) isHighlighted {
	return FLBitTest(_controlState, FLControlStateHighlighted);
}

- (void) setHighlighted:(BOOL) highlighted {
	FLBitChange(_controlState, FLControlStateHighlighted, highlighted);
}

- (BOOL) isSelected {
    return FLBitTest(_controlState, FLControlStateSelected);
}

- (BOOL) isDoubleSelected {
    return FLBitTest(_controlState, FLControlStateDoubleSelected);
}

- (void) setSelected:(BOOL) selected {
    FLBitChange(_controlState, FLControlStateSelected, selected);
}
...
- (BOOL) isDisabled {
	return FLBitTest(_controlState, FLControlStateDisabled);
}

- (void) setDisabled:(BOOL) disabled {
    FLBitChange(_controlState, FLControlStateDisabled, disabled);
}
