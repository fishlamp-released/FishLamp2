...
@implementation UIDevice (GtExtras)
...
{
	return GtToolbarHeightPortrait;
}

- (CGFloat) toolbarHeightLandscape
{
	return GtToolbarHeightLandscape;
}

- (CGFloat) navigationBarHeight;
{
	return DeviceIsPad() ?
		GtNavigationBarHeightPortrait :
...
{
	return GtNavigationBarHeightPortrait;
}

- (CGFloat) navigationBarHeightLandscape
{
	return GtNavigationBarHeightLandscape;
}
...
{
	return [UIDevice memorySizeInBytes]/GtMegabtyes > 128;
}
...
- (BOOL) isPhone
{
	return [[self platform] rangeOfString:@"iPhone"] .length > 0;
}
- (BOOL) isPad
{
...
void GetMemoryStats(unsigned long long* totalMemory, unsigned long long* used, unsigned long long* freeMem)
{
	...
	{
	   GtLog(@"Failed to fetch vm statistics");
	   return;
	}
