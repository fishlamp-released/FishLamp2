...
BOOL FLCompareDicts(NSDictionary* lhsDict, NSDictionary* rhsDict)
{
	...
	for(NSString* key in lhsDict)
	{
		...
		{
			equal = NO;
			FLCDebugLog(@"rhs missing %@/%@", key, lhsValue);
		}
		else if(!FLStringsAreEqual(lhsValue, rhsValue))
		{
			equal = NO;
			FLCDebugLog(@"value for key %@ are not equal: %@ != %@", key, lhsValue, rhsValue);
		}
		else 
		{
			FLCDebugLog(@"found %@=%@ in both headers", key, lhsValue);
		}
	}
...
void FLDebugCompareStrings(NSString* lhs, NSString* rhs)
{
	...
	{
		FLCDebugLog(@"lhs length: %d != rhs length: %d", lhs.length, rhs.length);
	}
	...
	for(int i = 0; i < (int) MAX(lhs.length, rhs.length); i++)
	{
		if(i >= (int)lhs.length)
		{
			FLCDebugLog(@"lhs ran out of chars");
			FLCDebugLog(str);
		}
		else if(i >= (int)rhs.length)
		{
			FLCDebugLog(@"rhs ran out of chars");
			FLCDebugLog(str);
		}
		...
		{
			FLCDebugLog(str);
			FLCDebugLog(@"%d: %c != %c", i, [lhs characterAtIndex:i], [rhs characterAtIndex:i]); 
			return;
		}
	}
...
//	
////	FLCDebugLog(@"Keys are equal: %d", FLStringsAreEqual(@"8wUi7m5HFQy76nowoCThusfgB+Q=", key));
//
//	FLCDebugLog(key);
//
