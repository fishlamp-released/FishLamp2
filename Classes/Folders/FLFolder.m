...
- (void) setFolderPath:(NSString*) folderPath {
	FLAssertStringIsNotEmpty(folderPath);
	FLAssignObject(_fullPath, folderPath);
}
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    self = [super init];
    if(self) {
        FLAssignObject(_fullPath, [aDecoder decodeObjectForKey:@"_fullPath"]);
    }   
		...
		FLPerformBlockInAutoreleasePool(^{
			...
			for(NSString* path in everything) {
				FLPerformBlockInAutoreleasePool(^{
                    ...
                    if(innerErr) {
                       FLThrowError(FLReturnAutoreleased(innerErr));
                    }
...
- (NSData*) readDataFromFile:(NSString*) fileName {
	...
	if(![[NSFileManager defaultManager] fileExistsAtPath:path]) {
		FLThrowError( FLReturnAutoreleased([[NSError alloc] initWithDomain:NSCocoaErrorDomain code:NSFileNoSuchFileError userInfo:nil]));
	}
    ...
    if(internalErr) {
        FLThrowError(FLReturnAutoreleased(internalErr));
    }

    if(!data) {
        FLThrowErrorCode(NSCocoaErrorDomain, NSFileReadUnknownError, @"unexpectedly read empty file");
    }
