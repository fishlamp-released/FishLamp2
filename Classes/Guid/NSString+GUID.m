...
+ (NSString*) guidString {
	...
	NSString* str = (__fl_bridge_transfer NSString*) CFUUIDCreateString(kCFAllocatorDefault, uuid);
	CFRelease(uuid);
	...
	if(!s_zero_guid) {
		@synchronized(self) {
			if(!s_zero_guid) {
				...
				s_zero_guid = (__fl_bridge NSString*) CFUUIDCreateString(kCFAllocatorDefault, uuid);
				CFRelease(uuid);
			}
		}
	}
