...
- (void) setTouchHandler:(FLTouchHandler*) touchHandler {
    ...
    _touchHandler.onSelected = ^(id widget) {
        [self.view widgetWasTouched:self];
    };
}
...
- (void) visitWidgets:(void (^)(id widget)) visitor {
	FLAssertIsNotNil(visitor);
    if(visitor)
    {
        for(FLWidget* widget in self.subwidgets)
        {
            [widget visitWidgets:visitor];
            visitor(widget);
        }
    }
}
...
- (void) _updateParentHidden {
    self.parentHidden = [_parent isHidden];
    [self visitWidgets:^(id widget){
...
- (void) setControlStateMask:(FLControlStateMask) controlState {
    if(controlState != self.controlStateMask) {

		for(FLWidget* widget in _widgets) {
			widget.controlStateMask = controlState;
		}

        [super setControlStateMask:controlState];
    }
}
...
- (void) removeWidget:(FLWidget*) widget {
    ...
    if(_widgets && widget.parent == self) {
        FLReturnAutoreleased(FLReturnRetained(widget));
...
- (void) removeFromParent {
    if(_parent) {
        [_parent willRemoveWidget:self];
        [_parent removeWidget:self];
        [self didMoveToParent];
    }
}

- (void) drawSelf:(CGRect) rect {
    FLAssertIsNotNil(self.parent);
...
- (UIView*) view {
    return _parent ? [_parent view] : nil;
}
...
- (id) lastSubwidgetByWeight:(FLArrangeableWeight) weight {
    return [FLArrangeableObject lastSubframeByWeight:weight subframes:self.subwidgets];
}
