...
- (void) setControlState:(FLControlState*) state {
    ...
    [_rootWidget.controlState removeControlStateObserver:self];
    _rootWidget.controlState = state;
    [_rootWidget.controlState addControlStateObserver:self];
}
...
- (id) objectByMoniker:(id) aMoniker {
    id outObj = [super objectByMoniker:aMoniker];
    if(!outObj) {
        outObj = [_rootWidget objectByMoniker:aMoniker];
    }
