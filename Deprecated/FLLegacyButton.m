...
- (void) setEnabled:(BOOL) enabled
{
	...
	if(_image)
	{
		...
		else
		{
			if(!_disabledImage)
			{
				_disabledImage = [[_image imageTintedWithColor:[UIColor grayColor] fraction:0] retain];
			}
...
//	  FLAssert(_imageView.isFrameOptimized, @"image view frame not optimized");	
//	  FLAssert(_titleLabel.isFrameOptimized, @"title view frame not optimized");	 
}
