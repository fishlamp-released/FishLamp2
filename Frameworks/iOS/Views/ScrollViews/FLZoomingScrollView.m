...
- (void) setZoomedView:(UIView*) zoomedView
{
	if(_zoomedView != zoomedView)
	{
		...
		if(zoomedView)
		{
			...
			_zoomedView = FLRetain(zoomedView);
...
//	  
	if(self.animating)
	{
		[UIView beginAnimations:@"viewin" context:(void*)CFBridgingRetain((target && action) ?
			[FLCallbackObject callbackWithContainedTarget:[FLRetainedObject retainedObject:target] action:action] :
			nil)];
		[UIView setAnimationDuration:0.3];
		[UIView setAnimationCurve:UIViewAnimationCurveEaseInOut];
		[UIView setAnimationDelegate:self];
