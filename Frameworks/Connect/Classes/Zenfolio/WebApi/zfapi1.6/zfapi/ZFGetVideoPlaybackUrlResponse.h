//	This file was generated at 3/10/12 1:14 PM by PackMule. DO NOT MODIFY!!
//
//	ZFGetVideoPlaybackUrlResponse.h
//	Project: FishLamp WebAPI
//	Schema: ZFApi1_6
//
//	Copywrite 2011 GreenTongue Software, LLC. All rights reserved.
//

/*
This is Zenfolio Web API. See <A href="/Zenfolio/help/api">help</A> for details.
*/



// --------------------------------------------------------------------
// ZFGetVideoPlaybackUrlResponse
// --------------------------------------------------------------------
@interface ZFGetVideoPlaybackUrlResponse : NSObject<NSCoding, NSCopying>{ 
@private
	NSString* _GetVideoPlaybackUrlResult;
} 


@property (readwrite, retain, nonatomic) NSString* GetVideoPlaybackUrlResult;

+ (NSString*) GetVideoPlaybackUrlResultKey;

+ (ZFGetVideoPlaybackUrlResponse*) getVideoPlaybackUrlResponse; 

@end

@interface ZFGetVideoPlaybackUrlResponse (ValueProperties) 
@end

