...
- (void) _didDeleteGroupElement:(FLSynchronousOperation*) operation
{
	...
	{
		ZFGroupElement* groupElement = self.currentQueueObject;
	
		ZFGroup* parentGroup = [ZFGroup loadFromCacheWithId:groupElement.parentGroupId.intValue];
		if(parentGroup)
		{
...
{
	ZFGroupElement* groupElement = object;
	...
	{		 
		ZFApiSoapDeleteGroup* groupOp = [[ZFApiSoapDeleteGroup alloc] initWithServerContext:[ZFApiSoap instance]];
...
//		[groupOp setDidPerformCallback:self action:@selector(_didDeleteGroupElement:)];
	...
	{
		ZFApiSoapDeletePhotoSet* psOp = [[ZFApiSoapDeletePhotoSet alloc] initWithServerContext:[ZFApiSoap instance]];
