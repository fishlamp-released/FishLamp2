...
- (void) _didUpdatePhotoMetaData:(ZFApiSoapUpdatePhoto*) operation {
	if(operation.didSucceed) {
		...
		ZFPhotoSet* photoSet = [[[self.userContext photoSetStorage] loadPhotoSetWithID:photoSetId];
...
- (void) _beginUploadingMetaData {
    ...
    if(!canSkipUpload) {
        if(self.photo.uploadedAssetId == 0) {
            FLThrowErrorCodeWithComment(ZFErrorDomain, errUnknownObjectId, @"Photo id is zero");
        }
