...
#import "FLZfBatchDeleteGroupElements.h"
#import "FLAction.h"
#import "FLZfUtils.h"
...
@implementation FLZfBatchDeleteGroupElements

- (void) _didDeleteGroupElement:(FLOperation*) operation
{
	if(operation.didSucceed)
	{
		FLZfGroupElement* groupElement = self.currentQueueObject;
	
		FLZfGroup* parentGroup = [FLZfGroup loadFromCacheWithId:groupElement.parentGroupId.intValue];
		if(parentGroup)
		{
...
{
	FLZfGroupElement* groupElement = object;
	...
	{		 
		FLZfApiSoapDeleteGroup* groupOp = [[FLZfApiSoapDeleteGroup alloc] initWithServerContext:[FLZfApiSoap instance]];
...
//		[groupOp setDidPerformCallback:self action:@selector(_didDeleteGroupElement:)];
	...
	{
		FLZfApiSoapDeletePhotoSet* psOp = [[FLZfApiSoapDeletePhotoSet alloc] initWithServerContext:[FLZfApiSoap instance]];
