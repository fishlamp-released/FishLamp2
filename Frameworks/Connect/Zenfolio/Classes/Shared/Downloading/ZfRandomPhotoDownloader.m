...
- (void) beginLoadingPhoto:(ZFPhoto*) photo
{
    ...
    ZFLoadImageFromCacheOperation* operation = FLAutorelease([[ZFLoadImageFromCacheOperation alloc] initWithSubOperation:loadPhotoHttpRequest]);
...
- (void) _didLoadRandomGroupElement:(ZFGroupElement*) groupElement
{
    ...
    if(groupElement.isGroupElement)
    {
        ...
        for(NSInteger i = (NSInteger) subGroups.count-1; i >= 0; i--)
        {
            ...
            {
                ZFPhotoSet* photoSet = [[[self.context photoSetStorage] loadPhotoSetWithID:subElement.IdValue];
                if(photoSet)
                {
