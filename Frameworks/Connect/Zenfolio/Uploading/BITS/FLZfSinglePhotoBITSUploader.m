...
#import "FLZfSinglePhotoBITSUploader.h"

#import "FLZfBITSCreateSessionOperation.h"
#import "FLZfBITSCloseSessionOperation.h"
#import "FLZfBITSUploadFragment.h"

#define kBitsFragmentSize (200 * 1024)


@interface FLZfSinglePhotoBITSUploader ()
- (void) _beginUploadingNextFragment;
@end

@implementation FLZfSinglePhotoBITSUploader
...
- (void) _beginCloseSession
{
    ...
    [action addOperation:FLAutorelease([[FLZfBITSCloseSessionOperation alloc] initWithUploadablePhoto:self.photo 
...
- (void) _didUploadFragment:(FLAction*) action
{
    if(action.didSucceed)
    {
        FLZfBITSUploadFragment* uploader = [action lastOperation];
...
- (BOOL) hasExpired
{
    return _startTime != 0.0f && ([NSDate timeIntervalSinceReferenceDate] - _startTime) > kZfSinglePhotoBITSUploaderExpireTime;
}
...
- (void) _beginUploadingNextFragment {
    ...
    FLAction* action = [self createAction];
    [action addOperation:FLAutorelease([[FLZfBITSUploadFragment alloc] initWithUploadablePhoto:self.photo 
...
- (void) _beginBitsSession
{
    ...
    [action addOperation:FLAutorelease([[FLZfBITSCreateSessionOperation alloc] initWithUploadablePhoto:self.photo])];
