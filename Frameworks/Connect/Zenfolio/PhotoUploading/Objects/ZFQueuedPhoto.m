...
@implementation ZFQueuedPhoto (FishLamp)
...
- (NSURL*) buildUploadURL:(BOOL) includeParameters
{
    if(FLStringIsEmpty(self.uploadGallery.uploadUrl)) {
        FLThrowErrorCodeWithComment(ZFErrorDomain, ZFErrorCodeUploadPhotoSetNotFound, @"PhotoSet not found: %@", self.uploadGallery.name);
    }
