//	This file was generated at 3/10/12 1:14 PM by PackMule. DO NOT MODIFY!!
//
//	ZFApiHttpGetLoadPrivateProfile.h
//	Project: FishLamp WebAPI
//	Schema: ZFApi1_6
//
//	Copywrite 2011 GreenTongue Software, LLC. All rights reserved.
//

/*
This is Zenfolio Web API. See <A href="/Zenfolio/help/api">help</A> for details.
*/

/*
Loads the private profile of the current user. <A href="/Zenfolio/help/api/ref/methods/loadprivateprofile">More...</A>
*/



@class ZFLoadPrivateProfileHttpGetIn;
@class ZFUser;

// --------------------------------------------------------------------
// ZFApiHttpGetLoadPrivateProfile
// --------------------------------------------------------------------
@interface ZFApiHttpGetLoadPrivateProfile : NSObject{ 
@private
	ZFLoadPrivateProfileHttpGetIn* _input;
	ZFUser* _output;
} 


@property (readwrite, retain, nonatomic) ZFLoadPrivateProfileHttpGetIn* input;
// Getter will create _input if nil. Alternately, use the inputObject property, which will not lazy create it.

@property (readwrite, retain, nonatomic) ZFUser* output;
// Getter will create _output if nil. Alternately, use the outputObject property, which will not lazy create it.

+ (NSString*) inputKey;

+ (NSString*) outputKey;

+ (ZFApiHttpGetLoadPrivateProfile*) apiHttpGetLoadPrivateProfile; 

@end

@interface ZFApiHttpGetLoadPrivateProfile (ValueProperties) 
@end


@interface ZFApiHttpGetLoadPrivateProfile (ObjectMembers) 

@property (readonly, retain, nonatomic) ZFLoadPrivateProfileHttpGetIn* inputObject;
// This returns _input. It does NOT create it if it's NIL.

@property (readonly, retain, nonatomic) ZFUser* outputObject;
// This returns _output. It does NOT create it if it's NIL.

- (void) createInputIfNil; 

- (void) createOutputIfNil; 
@end

