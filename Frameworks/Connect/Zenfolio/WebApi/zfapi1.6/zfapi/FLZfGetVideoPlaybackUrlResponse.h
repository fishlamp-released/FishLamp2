//	This file was generated at 3/10/12 1:14 PM by PackMule. DO NOT MODIFY!!
//
//	FLZfGetVideoPlaybackUrlResponse.h
//	Project: FishLamp WebAPI
//	Schema: FLZfApi1_6
//
//	Copywrite 2011 GreenTongue Software, LLC. All rights reserved.
//

/*
This is Zenfolio Web API. See <A href="/zf/help/api">help</A> for details.
*/



// --------------------------------------------------------------------
// FLZfGetVideoPlaybackUrlResponse
// --------------------------------------------------------------------
@interface FLZfGetVideoPlaybackUrlResponse : NSObject<NSCoding, NSCopying>{ 
@private
	NSString* _GetVideoPlaybackUrlResult;
} 


@property (readwrite, retain, nonatomic) NSString* GetVideoPlaybackUrlResult;

+ (NSString*) GetVideoPlaybackUrlResultKey;

+ (FLZfGetVideoPlaybackUrlResponse*) getVideoPlaybackUrlResponse; 

@end

@interface FLZfGetVideoPlaybackUrlResponse (ValueProperties) 
@end

