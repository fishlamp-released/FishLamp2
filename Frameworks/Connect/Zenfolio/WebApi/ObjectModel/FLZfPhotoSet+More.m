...
@implementation FLZfPhotoSet (More)

FLSynthesizeCachedObjectHandlerProperty(FLZfPhotoSet);
...
- (BOOL) removePhoto:(FLZfPhoto*) photoToRemove
 forceCountDecrement:(BOOL) forceCountDecrement {
	...
	if(self.Photos) {
		...
		for(NSUInteger i = 0; i < array.count; i++) {
			FLZfPhoto* photo = [array objectAtIndex:i];
...
- (NSUInteger) countLoadedPhotos {
	...
	for(FLZfPhoto* photo in self.Photos) {
		if(photo.IdValue == 0){
			break;
		}
		++loadedCount;
