...
@implementation ZFBatchDeletePhotos
...
- (void) onUpdateCacheAfterDeletedPhoto:(FLOperation*) operation
{
	...
	{
		ZFPhoto* photo = self.currentQueueObject;
...
- (FLAction*) createActionWithCurrentQueueObject:(id) object
{
	...
	ZFPhotoSetType type = _photoSet.TypeValue;
	if(type == ZFPhotoSetTypeGallery)
	{
		ZFApiSoapDeletePhoto* operation = [[ZFApiSoapDeletePhoto alloc] initWithServerContext:[ZFApiSoap instance]];
...
//		[operation setDidPerformCallback:self action:@selector(onUpdateCacheAfterDeletedPhoto:)];
	...
	{
		ZFApiSoapCollectionRemovePhoto* operation = [[ZFApiSoapCollectionRemovePhoto alloc] initWithServerContext:[ZFApiSoap instance]];
