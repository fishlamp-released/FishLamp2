//
//  FLZenfolioBatchPhotoDownloadOperation.m
//  ZenfolioDownloader
//
...
@interface FLZenfolioBatchPhotoDownloadOperation()

- (FLStorableImage*) cachedOriginalImageForPhoto:(FLZenfolioPhoto *)photo;

- (void)setCacheDataPath:(NSString *)path forPhoto:(FLZenfolioPhoto *)photo;

@property (readwrite, strong, nonatomic) NSMutableDictionary* photoCache;
@property (readwrite, assign, getter=isPaused) BOOL paused;
@property (readwrite, strong) FLZenfolioPhotoSet* currentSet;
@property (readwrite, strong) FLZenfolioPhoto* currentPhoto;
...
@implementation FLZenfolioBatchPhotoDownloadOperation
...
- (id) initWithRootGroup:(FLZenfolioGroup*) rootGroup   destination:(NSString*) destination {
	self = [super init];
    if(self){
...
- (NSString*)uniquePathForEntry:(FLZenfolioGroupElement *)entry 
    ...
    if(!unique) {
        unique = FLZenfolioUniquePath([folder stringByAppendingPathComponent:[entry Title]]);
...
- (void) notifyDownloadedPhoto:(FLZenfolioPhoto*) photo {
