...
@implementation FLColor (FLExtras)

+ (FLColor*) colorWithCSSStyleString:(NSString*) string
{
    if(string.length == 7 && [string characterAtIndex:0] == '#')
    {
        ...
        return [FLColor colorWithRgbValues:red green:green blue:blue alpha:1.0f];
    }
...
{
    FLColor* color = [FLColor colorWithCSSStyleString:string];
    if(!color)
    {
        CGFloat rgb[4] = {
            0,
        ...
        *p = 0;
        ...
        if([string rangeOfString:@"rgb("].location == 0)
        {
            ...
            color = [FLColor colorWithRgbValues:rgb[0] green:rgb[1] blue:rgb[2] alpha:rgb[3]];
        } 
    }
...
+ (FLColor*) colorWithRgbValues:(CGFloat) red 
	green:(CGFloat) green 
	blue:(CGFloat) blue 
	alpha:(CGFloat) alpha
{
#if IOS
	return [FLColor colorWithRed:red/255.0f 
								green:green/255.0f 
								blue:blue/255.0f 
								alpha:alpha ];
#else
	return [FLColor colorWithDeviceRed:red/255.0f 
								green:green/255.0f 
								blue:blue/255.0f 
								alpha:alpha ];
#endif                                
