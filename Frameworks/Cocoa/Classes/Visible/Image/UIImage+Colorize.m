...
@implementation UIImage_ (FLColorize)
...
-(NSImage_ *)colorizeImage:(UIColor_ *)theColor blendMode:(CGBlendMode) blendMode
{
	...
	NSImage_ *newImage = UIGraphicsGetImageFromCurrentImageContext();
...
+ (NSImage_*) imageNamed:(NSString*) name withColor:(UIColor_*) color blendMode:(CGBlendMode) blendMode {
    NSImage_* image = [NSImage_ imageNamed:name];
    FLAssertIsNotNil_v(image, nil);

	return [image colorizeImage:color blendMode:blendMode];
}

+ (NSImage_*) whiteImageNamed:(NSString*) name {
	return [NSImage_ imageNamed:name withColor:[UIColor_ whiteColor] blendMode:kCGBlendModeOverlay];
}
