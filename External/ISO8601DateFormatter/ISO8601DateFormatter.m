...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	NSCalendar *calendar = GtReturnAutoreleased([[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar]);
	calendar.firstWeekday = 2; //Monday
	NSDate *now = [NSDate date];

	NSDateComponents *components = GtReturnAutoreleased([[NSDateComponents alloc] init]);
...
- (NSString *) replaceColonsInString:(NSString *)timeFormat withTimeSeparator:(unichar)timeSep {
	if (timeSep != ':') {
		NSMutableString *timeFormatMutable = GtReturnAutoreleased([timeFormat mutableCopy]);
