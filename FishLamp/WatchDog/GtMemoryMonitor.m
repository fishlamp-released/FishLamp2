...
id GtMemoryMonitorUpdate(id obj)
{
    [GtMemoryMonitor update:obj];
    return obj;
}

void GtMemoryMonitorRelease(id obj)
{
#if DEBUG
    GtCheckRetainCount(obj);
#endif
    GtMemoryMonitorUpdate(obj);
    [obj release];
}
...
- (void) updateDisplayLabel
{
	...
	{   
        ...
        m_rightLabel.frame = AdjustFrame(GtBottomRightJustifyRectInRect(superviewBounds, m_rightLabel.frame));
        m_leftLabel.frame = AdjustFrame(GtBottomJustifyRectInRect(superviewBounds, m_leftLabel.frame));
	}
}
#if __clang__
SDF;
...
+ (id) update:(id) object
{
    [[GtMemoryMonitor instance] updateMemoryValues];
	return object;
}
...
//	GtBytesDataSizeFormatter* formatter = [[GtAlloc(GtBytesDataSizeFormatter) init] autorelease];
