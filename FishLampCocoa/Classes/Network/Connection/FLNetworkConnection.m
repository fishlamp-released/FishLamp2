...
- (FLFinisher*) openConnection:(FLDispatchQueue*) dispatcher {
    ...
    return [dispatcher dispatch:^(FLFinisher* finisher) {
        ...
        [self.networkStream openStream:dispatcher withResultBlock:^(id result) {
...
//- (id) runSynchronously:(id<FLDispatcher>) inDispatcher {
//    return [[self openConnection:inDispacher] waitUntilFinished]
//
//
//    return [self runSynchronously:inDispatcher withAsyncTask:[FLFinisher finisher]];;
//}
//
////- (id) runSynchronously:(id<FLDispatcher>) inDispatcher withAsyncTask:(id) asyncTask {
