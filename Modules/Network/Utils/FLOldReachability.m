...
- (void) onReachabilityCallback
{
	...
	{
		if( FLBitTest(newFlags, _broadcastMask) !=
			FLBitTest(_lastFlags, _broadcastMask))
		{
...
{
	FLAssert(info != NULL, @"info was NULL in ReachabilityCallback");
	FLAssert([(NSObject*) info isKindOfClass: [FLReachability class]], @"info was wrong class in ReachabilityCallback");
