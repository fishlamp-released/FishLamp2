...
- (void) initScrollView
{
	FLAssert(self != nil, @"scroll view is nil");
...
{
	FLAssert(which >= 0 && which < _tiledViews.count, @"bad idx");
...
{
	FLAssert(idx >= 0, @"view not found in arrangement");
...
{
	FLAssert(tileCount >= 3, @"expecting at least 3 tiles");
	FLAssert(tileCount % 2 == 1, @"expecting odd number of tiles");
	FLAssertIsNotNil(_tilingScrollViewDelegate);
