...
- (CGSize)sizeThatFits:(CGSize) inSize {
    ...
    UIEdgeInsets padding = self.arrangeableInsets;
...
- (void) layoutSubviews {
    ...
    if(!_buttonBox.hidden) {
        ...
        [_buttonBox layoutSubviewsWithArrangement:_buttonBox.subviewArrangement
                                   adjustViewSize:YES];
...
- (void) dealloc {
    ...
    FLRelease(_colorBar);
    FLRelease(_title);    [_messageLabel release];
    [super dealloc];
}
