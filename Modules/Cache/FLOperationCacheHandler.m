...
{
    if(FLBitTest(self.cacheBehavior, FLHttpOperationCacheBehaviorLoad))
	{
		id object = nil;
		...
		if(self.wasLoadedFromCache && !FLBitTest(self.cacheBehavior, FLHttpOperationCacheBehaviorContinueAfterLoad))
		{
			operation.shouldPerform = NO;
		}
...
{
    if(!operation.error && 
        FLBitTest(self.cacheBehavior, FLHttpOperationCacheBehaviorSave) && 
