...
#define AssignToVoid(s) ((__fl_bridge void*) s)
...
FLSplitKeyPath FLSplitKeyPathFromString(NSString* keyPathString)
{
	...
	if(range.length == 0)
	{
		...
		keyPath.dataKey = (__fl_bridge void*)FLReturnAutoreleased(FLReturnRetained(keyPathString));
	}
	else 
	{
		keyPath.dataSourceKey = (__fl_bridge void*)[keyPathString substringToIndex:range.location];
		keyPath.dataKey = (__fl_bridge void*)[keyPathString substringFromIndex:range.location+1];
	}
...
//@implementation FLObject (FLLegacyDataSourceAdaptor)
//
//- (BOOL) dataSourceManager:(FLLegacyDataSource*) dataSourceManager ownsObjectForKey:(id) key
//{
