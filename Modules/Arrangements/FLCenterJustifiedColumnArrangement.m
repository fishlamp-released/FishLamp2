...
- (FLSize) layoutArrangeableObjects:(NSArray*) objects
    ...
    for(id object in objects.reverseObjectEnumerator) {
		...
		right = FLRectGetRight([self arrangeableFrameForObject:object]);
        break;
    }
    ...
    if(right < size.width) {
        ...
        for(id object in objects) {
            ...
            [self setArrangeableFrame:FLRectMoveHorizontally([self arrangeableFrameForObject:object], offset)
                            forObject:object];
        }
    }
