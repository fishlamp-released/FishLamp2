...
{
	FLRelease(m_builder);
	FLRelease(m_masterExif);
	FLRelease(m_extras);
	FLRelease(m_overrides);
	FLSuperDealloc();
}
...
{
	[dataSourceManager setDataSource:m_extras forKey:FLCameraExifExtrasDataSourceKey];
...
{
	m_builder = [[FLTableViewLayoutBuilder alloc] initWithTableLayout:builder.tableLayout];
...
{
	for(FLEditObjectTableViewCell* row in m_builder.cells)
	{
