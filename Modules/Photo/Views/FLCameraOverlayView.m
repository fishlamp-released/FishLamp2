...
- (id) initWithFrame:(CGRect) frame
{
	if(self = [super initWithFrame:frame])
	{
		...
		m_leftButton.frame = FLRectSetSize(m_leftButton.frame, 60, 36);
		...
		m_centerButton.frame = FLRectSetSize(m_centerButton.frame, 160, 54);
		...
		m_rightButton = [[FLThumbnailButton alloc] initWithFrame:CGRectMake(0,0,m_leftButton.frame.size.height,m_leftButton.frame.size.height)];
		...
		m_zoomSlider.frame = FLRectSetSize(m_zoomSlider.frame, 300, 20);
		m_zoomSlider.transform = CGAffineTransformMakeRotation(FLDegreesToRadians(90));
		[self addSubview:m_zoomSlider];
		
		m_focusView = [[FLRoundRectView alloc] initWithFrame:CGRectMake(0,0,80,60)];
		...
		crosshairs.frame = FLRectCenterRectInRect(m_focusView.bounds, crosshairs.frame);
...
//		  m_focusView.hidden = YES;
		...
		m_countView = [[FLCountView alloc] initWithFrame:CGRectZero];
		m_countView.count = 0;
...
//	  m_focusView.hidden = NO;
	if(!m_dragging)
	{
		m_focusView.frame = FLRectCenterOnPoint(m_focusView.frame, pt);
...
- (void) dealloc
{
	...
	FLReleaseWithNil(m_countView);
	FLReleaseWithNil(m_focusView);
	FLReleaseWithNil(m_imageView);
	FLReleaseWithNil(m_shakeyImageView);
	FLReleaseWithNil(m_zoomSlider);
	FLReleaseWithNil(m_flashSegmentedControl);
	FLReleaseWithNil(m_flashButton);
	FLReleaseWithNil(m_flipCameraButton);
...
- (void) layoutSubviews
{
	...
	m_leftButton.newFrame = FLRectCenterOnPoint(m_leftButton.frame, 
		CGPointMake((m_leftButton.frame.size.width/2.0) + 10, bottomRowCenter));
		
	m_centerButton.newFrame = FLRectCenterOnPoint(m_centerButton.frame,
		CGPointMake(self.bounds.size.width / 2.0, bottomRowCenter));
		
	m_rightButton.newFrame = FLRectCenterOnPoint(m_rightButton.frame,
		CGPointMake(self.bounds.size.width - (m_rightButton.frame.size.width/2.0) - 10, bottomRowCenter));
	
	m_shakeyImageView.newFrame = FLRectCenterRectInRectHorizontally(self.bounds, FLRectSetTop(m_shakeyImageView.frame, 100));

	m_zoomSlider.newFrame = FLRectCenterRectInRectVertically(self.bounds, FLRectSetLeft(m_zoomSlider.frame, self.bounds.size.width - 40) );
	
	m_spinner.newFrame = FLRectCenterRectInRect(self.bounds, m_spinner.frame);
	
	[m_flipCameraButton sizeToFit];
	CGRect flipFrame = FLRectAddWidth(m_flipCameraButton.frame, 10);
	m_flipCameraButton.newFrame = FLRectSetOrigin(flipFrame, self.bounds.size.width - flipFrame.size.width - 10, 10);	
	
	[m_flashButton sizeToFit];
	m_flashButton.newFrame = FLRectSetOrigin(FLRectAddWidth(m_flashButton.frame, 10), 10, 10);
   
	m_flashSegmentedControl.newFrame = FLRectSetOriginWithPoint( 
		FLRectSetHeight(m_flashSegmentedControl.frame, CGRectGetHeight(m_flashButton.frame)), m_flashButton.frame.origin);
   
	m_countView.newFrame = FLRectCenterOnPoint(m_countView.frame, FLRectGetTopRight(m_rightButton.frame));	  
}
...
{	
	...
	m_focusView.frame = FLRectCenterOnPoint(m_focusView.frame, [touches.anyObject locationInView:self]);
