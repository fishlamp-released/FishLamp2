...
static void * const kAuxiliaryViewControllerKey = (void*)&kAuxiliaryViewControllerKey;

- (FLAuxiliaryViewController*) auxiliaryViewController {
    FLAuxiliaryViewController* controller = objc_getAssociatedObject(self, kAuxiliaryViewControllerKey);
    return controller ? controller : self.parentViewController.auxiliaryViewController;
}

- (void) setAuxiliaryViewController:(FLAuxiliaryViewController*) auxiliaryViewController {
    objc_setAssociatedObject(self, kAuxiliaryViewControllerKey, auxiliaryViewController, OBJC_ASSOCIATION_ASSIGN);
}
...
#if FL_DEALLOC 
    [_createViewController release];
    [_addTouchableViewsCallback release];
...
//    self.areaButton.openButton = [FLButtonbarView createImageButtonByName:@"list_small.png" target:self action:@selector(showSearchPanel:)];
//    self.areaButton.closeButton = [FLButtonbarView createImageButtonByName:@"x.png" target:self action:@selector(hideSearchPanel:)];
