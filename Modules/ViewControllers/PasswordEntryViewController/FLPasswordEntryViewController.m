...
- (void) willConstructWithTableLayoutBuilder:(FLTableViewLayoutBuilder*) builder
{
	...
	if(FLBitTestAny(_options, FLPasswordEntryViewControllerOptionConfirmationRequired))
	{
		[builder addSection];
...
- (BOOL) willBeginSavingChanges
{
	...
	if(FLBitTestAny(_options, FLPasswordEntryViewControllerOptionPasswordRequired))
	{
		if(FLStringIsEmpty(password))
		{
			FLAlertViewController* alert = [FLAlertViewController alertViewController:NSLocalizedString(@"No password was entered.", nil) 
	...
	if(FLBitTestAny(_options, FLPasswordEntryViewControllerOptionConfirmationRequired))
	{
		...
		{
			FLAlertViewController* alert = [FLAlertViewController alertViewController:NSLocalizedString(@"The two passwords you entered do not match.", nil) 
