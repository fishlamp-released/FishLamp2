//
//  FLVisibleGridViewCellCollection.m
//  FishLamp-iOS-Lib
//
...
@implementation FLContiguousVisibleGridViewCellCollection
...
- (void) removeAllCells {
    ...
    [_delegate visibleGridViewCellCollectionVisibleCellsDidChange:self];
}
...
- (NSUInteger)countByEnumeratingWithState:(NSFastEnumerationState *)state 
    ...
    FLOrderedCollection* cells = [_delegate visibleGridViewCellCollectionGetCellCollection:self];
...
// ug. a loop within a loop is never good.                
                    for(NSInteger i = backwardIndex - 1; i >= 0; i--) {
                        ...
                        FLGridViewCell* cell = [array objectAtIndex:i];
        ...
        FLAssert(newFirstIndex != NSNotFound, @"didn't find first cell");
...
- (void) recalculateVisibleCellsInBounds:(CGRect) visibleBounds  {
    ...
    [_delegate visibleGridViewCellCollection:self updateCellVisiblityInBounds:visibleBounds];
    ...
    for(int i = 0; i < count; i++) {
        FLGridViewCell* cell = [cells cellAtIndex:i];
    
        FLAssert(cell.visible == CGRectIntersectsRect(cell.frame, visibleBounds), @"Rect visibility for cell %d is wrong", i);
    }
#endif
