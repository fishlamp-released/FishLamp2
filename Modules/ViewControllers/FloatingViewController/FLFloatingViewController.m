...
@interface FLFloatingViewEventReceiver : NSObject<FLEventInterceptor> {
@private
...
- (void) _setFramePosition:(CGRect) frame animated:(BOOL) animated
{
	...
	if(view.superview)
	{
		...
		{
			FLAssert([_positionProvider respondsToSelector:@selector(frame)], @"must implement frame");
			FLAssert([_positionProvider respondsToSelector:@selector(superview)], @"must implement frame");
...
{
    FLAssert(!FLSizeIsEmpty(size), @"empty size");
    FLAssert(size.height > 0, @"invalid height");
    FLAssert(size.width > 0, @"invalid width");
...
static void * const kSizeKey = (void*)&kSizeKey;
...
{
    NSValue* value = objc_getAssociatedObject(self, kSizeKey);
...
{
    objc_setAssociatedObject(self, kSizeKey, [NSValue valueWithFLSize:size], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
{
    FLFixMe(@"present floating view controller not implemented");
...
- (FLFloatingViewController*) presentModalFloatingViewController:(UIViewController*) controller
    animated:(BOOL) animated
{
    FLFixMe(@"present modal floating view controller not implemented");
