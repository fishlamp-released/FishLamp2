...
{
	FLAssertIsNotNil(_tabBar);
	FLAssert(_tabBar == tabBar, @"different tabBar?");

	NSUInteger idx = [_tabBar indexForTabBarItem:item];
	
	FLAssert(idx >= 0 && idx < tabBar.items.count, @"didn't find item");
...
// hand waving required to set badge on unselected item.
	NSArray* items = FLReturnAutoreleased(FLReturnRetained(self.items));
