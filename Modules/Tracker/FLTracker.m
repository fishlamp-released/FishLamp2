...
- (void) _logEvent:(FLTrackerLevel) trackLevel
    ...
    [[FLFifoQueue instance] queueBlock:^{
        if( !_disabled && 
            FLBitTest(_subscribedEvents, eventType) &&
            FLBitTest(_trackLevel, trackLevel)) {

            for(id<FLTrackerSink> sink in _sinks) {
                if( FLBitTest(sink.subscribedEvents, eventType) &&
                    FLBitTest(sink.trackLevel, trackLevel) ) {
                    block(self, sink); 
                }
            }
        }
